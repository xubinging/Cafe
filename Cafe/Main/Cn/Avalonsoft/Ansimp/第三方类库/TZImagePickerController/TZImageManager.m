...
-(void)getPhotoBytesWithModel:(TZAssetModel *)model completion:(void (^)(NSInteger))completion {
    __block NSInteger dataLength = 0;
    [self getOriginalPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info) {
...
-(PHImageRequestID)asynGetOriginalPhotoDataWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *photo,NSDictionary *info))completion {
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc]init];
    option.networkAccessAllowed = YES;
    option.resizeMode = PHImageRequestOptionsResizeModeFast;
    return [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage *result, NSDictionary *info) {
