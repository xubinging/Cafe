...
#import "AvalonsoftAreaPickerView.h"
...
static NSInteger const ScrollViewY = 80;    //ScrollViewY Y坐标起始位
static NSInteger const PickerViewHeight = 600;

@interface AvalonsoftAreaPickerView ()<AvalonsoftAreaPickerListViewDelegate,AvalonsoftAreaPickerHeaderViewDelegate,UIScrollViewDelegate>

@property (nonatomic , strong) UIScrollView *ScrollView;
@property (nonatomic , strong) UILabel *TitleLb;
@property (nonatomic , strong) AvalonsoftAreaPickerHeaderView *header;
...
@implementation AvalonsoftAreaPickerView
...
- (void)initForm
{
    ...
    self.TitleLb = [[UILabel alloc]initWithFrame:CGRectMake(0, 10,self.frame.size.width, 22)];
    self.TitleLb.textColor = [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0];
    self.TitleLb.textAlignment = NSTextAlignmentCenter;
    self.TitleLb.font = [UIFont fontWithName:@"PingFangSC-Medium" size: 16];
    [ContentView addSubview:self.TitleLb];
    
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(10, 11, 20, 20);
    [button setImage:[UIImage imageNamed:@"login_del_back"] forState:UIControlStateNormal];
    [button addTarget:self action:@selector(hiddenClick) forControlEvents:UIControlEventTouchUpInside];
    [ContentView addSubview:button];
    
    self.header = [[AvalonsoftAreaPickerHeaderView alloc]initWithFrame:CGRectMake(0, 45, self.frame.size.width, headerHeight)];
...
{
    self.dataArrays  = [self.dataArrays subarrayWithRange:NSMakeRange(0, AvalonsoftAreaPickerListView.tag + 1)].mutableCopy;
    self.headerDataArray  = [self.headerDataArray subarrayWithRange:NSMakeRange(0, AvalonsoftAreaPickerListView.tag)].mutableCopy;
    [self.headerDataArray addObject:value.name];
    [self.headerDataArray addObject:@"请选择"];
    
    NSMutableArray<AvalonsoftAreaPickerModel *> * array;
    if ([self.delegate respondsToSelector:@selector(AvalonsoftAreaPickerView:didSelcetedTier:selcetedValue:)]) {
        array =  [self.delegate AvalonsoftAreaPickerView:self didSelcetedTier:AvalonsoftAreaPickerListView.tag selcetedValue:value];
        if (array.count>0) {
...
- (void)setDataArray:(NSArray<AvalonsoftAreaPickerModel *> *)dataArray
{
...
- (void)manageDataArray:(NSArray *)dataArray  selectText:(NSString *)Text
{
    ...
    AvalonsoftAreaPickerListView *ListView = [[AvalonsoftAreaPickerListView alloc]initWithFrame:CGRectMake((self.frame.size.width *(self.dataArrays.count - 1)), 0, self.frame.size.width, PickerViewHeight  - ScrollViewY)];
    ...
    [tempArray enumerateObjectsUsingBlock:^(AvalonsoftAreaPickerListView * obj, NSUInteger idx, BOOL * _Nonnull stop) {
