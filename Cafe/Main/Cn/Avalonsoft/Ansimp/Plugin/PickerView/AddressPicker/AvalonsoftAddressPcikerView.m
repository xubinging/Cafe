...
#import "AvalonsoftAddressPcikerView.h"

#import "AvalonsoftAddressModel.h"

@interface AvalonsoftAddressPcikerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
// 选中后的回调
@property (nonatomic, copy) AvalonsoftAddressResultBlock resultBlock;

@end

@implementation AvalonsoftAddressPcikerView
...
{
    AvalonsoftAddressPcikerView *addressPickerView = [[AvalonsoftAddressPcikerView alloc] initWithTitle:title DefaultSelected:defaultSelectedArr FileName:nil IsAutoSelect:isAutoSelect Manager:nil ResultBlock:resultBlock];
...
+ (void)showAddressPickerWithTitle:(NSString *)title
                   DefaultSelected:(NSArray *)defaultSelectedArr
                      ...
                      IsAutoSelect:(BOOL)isAutoSelect
                       ...
                       ResultBlock:(AvalonsoftAddressResultBlock)resultBlock
{
    AvalonsoftAddressPcikerView *addressPickerView = [[AvalonsoftAddressPcikerView alloc] initWithTitle:title DefaultSelected:defaultSelectedArr FileName:fileName IsAutoSelect:isAutoSelect Manager:nil ResultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}
...
- (void)loadData {
    ...
    if (self.fileName) {
        ...
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"AvalonsoftAddressCity" ofType:@"plist"];
        arrData = [NSMutableArray arrayWithContentsOfFile:filePath];
    }
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    AvalonsoftProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
    AvalonsoftCityModel *cityModel = provinceModel.city[rowOfCity];
    if (component == 0) {
        //返回省个数
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {//省
        AvalonsoftProvinceModel *provinceModel = self.addressModelArr[row];
        showTitleValue = provinceModel.name;
    }
    if (component == 1) {//市
        AvalonsoftProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        AvalonsoftCityModel *cityModel = provinceModel.city[row];
        showTitleValue = cityModel.name;
    }
    if (component == 2) {//区
        AvalonsoftProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        AvalonsoftCityModel *cityModel = provinceModel.city[rowOfCity];
        AvalonsoftTownModel *townModel = cityModel.town[row];
        ...
        AvalonsoftProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
            if (rowOfCity < provinceModel.city.count) {
                ...
                if (rowOfTown < cityModel.town.count) {
                    AvalonsoftTownModel *townModel = cityModel.town[rowOfTown];
...
- (void)scrollToRow:(NSInteger)firstRow secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow {
    if (firstRow < self.addressModelArr.count) {
        ...
        AvalonsoftProvinceModel *provinceModel = self.addressModelArr[firstRow];
        if (self.defaultSelectedArr.count == 2) {
        ...
        if (self.defaultSelectedArr.count == 3) {
            if (secondRow < provinceModel.city.count) {
                rowOfCity = secondRow;
                [self.pickerView reloadComponent:1];
                AvalonsoftCityModel *cityModel = provinceModel.city[secondRow];
