...
#import "AvalonsoftActionSheet.h"
...
@interface AvalonsoftActionSheet()<AvalonsoftSheetViewDelegate>

@property (strong, nonatomic) UIButton *bgButton;
@property (strong, nonatomic) UIView *contentView;
@property (strong, nonatomic) AvalonsoftSheetView *sheetView;
@property (strong, nonatomic) AvalonsoftSheetHead *titleView;
@property (strong, nonatomic) AvalonsoftSheetFoot *footView;
...
@property (assign, nonatomic) AvalonsoftSheetStyle sheetStyle;

@end

@implementation AvalonsoftActionSheet
              ...
              itemTitles:(NSMutableArray *)itemTitles
{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        ...
        //取消按钮View
        self.footView = [[AvalonsoftSheetFoot alloc] init];
        [self addSubview:self.footView];
        //选择TableView
        self.sheetView = [[AvalonsoftSheetView alloc] init];
...
///初始化默认样式
- (id)upDefaultStyeWithItems:(NSMutableArray *)itemTitles title:(NSString *)title selfView:(AvalonsoftActionSheet *)selfView
{
    //半透明背景按钮
    ...
    BOOL isTitle = NO;
    if (title.length > 0) {
        selfView.titleView = [[AvalonsoftSheetHead alloc] init];
    ...
    //要考虑全面屏底部的区域 TabbarSafeBottomMargin
    selfView.footViewY = kScreenHeight - kCellH - kMargin - TabbarSafeBottomMargin;
    selfView.footView.frame = CGRectMake(kMargin, kScreenHeight + selfView.contentVH + kMargin - TabbarSafeBottomMargin, kScreenWW, kCellH);
    
    selfView.contentViewY = kScreenHeight - CGRectGetHeight(selfView.footView.frame) - selfView.contentVH - kMargin * 2 - TabbarSafeBottomMargin;
    selfView.contentView.frame = CGRectMake(kMargin, kScreenHeight- TabbarSafeBottomMargin, kScreenWW, selfView.contentVH);
...
///初始化微信样式
- (id)upWeiChatStyeWithItems:(NSMutableArray *)itemTitles title:(NSString *)title selfView:(AvalonsoftActionSheet *)selfView
{
...
///初始化TableView样式
- (id)upTableStyeWithItems:(NSMutableArray *)itemTitles title:(NSString *)title selfView:(AvalonsoftActionSheet *)selfView
{
...
- (void)show
{
    if (_sheetStyle == AvalonsoftSheetStyleDefault) {
        [self pushDefaultStyeSheetView];
    }
    else if (_sheetStyle == AvalonsoftSheetStyleWeiChat) {
        [self pushWeiChatStyeSheetView];
    }
    else if (_sheetStyle == AvalonsoftSheetStyleTable) {
        [self pushTableStyeSheetView];
    }
}
...
- (AvalonsoftSheetFoot *)footView
{
    
    if (!_footView) {
        _footView = [[AvalonsoftSheetFoot alloc] init];
        
    }
    return _footView;
    
}
- (AvalonsoftSheetHead *)titleView
{
    if (!_titleView) {
        _titleView = [[AvalonsoftSheetHead alloc] init];
    }
    return _titleView;
