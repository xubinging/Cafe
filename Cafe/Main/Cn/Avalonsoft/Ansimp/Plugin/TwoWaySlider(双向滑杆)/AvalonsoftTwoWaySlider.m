...
#import "AvalonsoftTwoWaySlider.h"

@implementation AvalonsoftTwoWaySlider
...
- (instancetype)initWithFrame:(CGRect)frame
              ...
              blockSpaceValue:(CGFloat)blockSpaceValue
{
    return [self initWithFrame:frame
                         image:[UIImage imageNamed:@"home_foreign_slider"]
                      minValue:minValue
                      maxValue:maxValue
               blockSpaceValue:blockSpaceValue];
}
              ...
              blockSpaceValue:(CGFloat)blockSpaceValue
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _minValue = 0.f;
        _maxValue = 1.f;
        _progressLeftSpace = 10.f;
        _progressHeight = 6.f;
        _progressRadius = 3.f;
        _lightColor = RGBA_GGCOLOR(32, 188, 255, 1);
        _darkColor = RGBA_GGCOLOR(238, 238, 238, 1);
        _showAnimate = YES;
        _progressWidth = self.frame.size.width - _progressLeftSpace * 2;
        _indicateViewOffset = 0;
        _indicateViewWidth = 31;
...
- (void)initSliderUI
{
    ...
    _minIndicateView = [[AvalonsoftTwoWaySliderIndicateView alloc] init];
    [_minIndicateView addGestureRecognizer:minIndicatePan];
    
    _maxIndicateView = [[AvalonsoftTwoWaySliderIndicateView alloc] init];
...
- (void)panMinAction:(UIPanGestureRecognizer *)pan
{
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (lastX > point.x) {
                [_minIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionRight];
            }else if (lastX < point.x){
                [_minIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionLeft];
            }
            lastX = point.x;
            break;
        }
        ...
        case UIGestureRecognizerStateCancelled:{
            [_minIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionNormal];
            break;
        }
        default:
            break;
...
- (void)panMaxAction:(UIPanGestureRecognizer *)pan
{
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (!_showAnimate) break;
            if (lastX > point.x) {
                [_maxIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionRight];
            }else if (lastX < point.x){
                [_maxIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionLeft];
            }
            lastX = point.x;
            break;
        }
        case UIGestureRecognizerStateEnded:
        case UIGestureRecognizerStateCancelled:{
            [_maxIndicateView setDirection:AvalonsoftTwoWayIndicateDirectionNormal];
            break;
        }
        default:
            break;
