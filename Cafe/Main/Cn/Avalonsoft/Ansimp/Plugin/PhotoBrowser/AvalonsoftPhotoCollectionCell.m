//
//  AvalonsoftPhotoCollectionCell.m
//  Cafe
//
...
@interface AvalonsoftPhotoCollectionCell()<UIGestureRecognizerDelegate>
@property (weak, nonatomic) UIGestureRecognizer *recognizer;
@end

@implementation AvalonsoftPhotoCollectionCell
...
{
//    AvalonsoftPhotoBrowserController *controller = (AvalonsoftPhotoBrowserController *)self.viewController;
//    if ([controller isKindOfClass:[AvalonsoftPhotoBrowserController class]]) {
//        [controller performSelector:@selector(singleTapDetected) withObject:nil];
//    }
}
...
//        }
//
//        AvalonsoftMsgAlertView *alertView = [AvalonsoftMsgAlertView showWithTitle:@"温馨提示" content:@"是否保存图片到相册?" buttonTitles:@[@"取消",@"确定"] buttonClickedBlock:^(NSInteger buttonIndex){
//
//            if(buttonIndex == 1){
...
- (void)createView
{
    ...
    self.photoView = [[AvalonsoftTapImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
