//
//  MineAddActivityViewController.m
//  Cafe
//
...
@interface MineAddActivityViewController () <UITextFieldDelegate, UITextViewDelegate>
{
...
@implementation MineAddActivityViewController
...
{
    if (!name.length) {
        [AvalonsoftToast showWithMessage:@"活动名称不能为空！"];
    } else if (!role.length) {
        [AvalonsoftToast showWithMessage:@"活动角色不能为空！"];
...
- (void)saveMineAddActivityList
{
    ...
    [AvalonsoftHasNetwork avalonsoft_hasNetwork:^(bool has) {
        ...
        if (has) {
            ...
            [[AvalonsoftHttpClient avalonsoftHttpClient] requestWithAction:COMMON_SERVER_URL actionName:MINE_MY_ACTIVITY_ADD method:HttpRequestPost paramenters:root prepareExecute:^{
