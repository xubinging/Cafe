...
#import "MineJobViewController.h"

#import "MineJobModel.h"
#import "MineJobTableViewCell.h"
#import "MineAddWorkViewController.h"
#import "MineJobDetailViewController.h"     //详情页

@interface MineJobViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MineJobViewController
...
- (void)viewDidLoad {
    ...
    [self initView];
    [self queryMineWorkList];
}

- (NSMutableArray *)jobArray
{
    return _jobArray?:(_jobArray = [NSMutableArray array]);
}
...
-(void)initNavigationView
{
    ...
    NSMutableAttributedString *titleLabelString = [[NSMutableAttributedString alloc] initWithString:@"工作经历" attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Medium" size: 17],NSForegroundColorAttributeName: [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0]}];
...
-(void)initView
{
    _jobTableView = [UITableView new];
    [self.view addSubview:_jobTableView];
    [_jobTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(navigationView.mas_bottom).offset(10);
        make.left.equalTo(self.view);
        make.right.equalTo(self.view);
        make.bottom.equalTo(self.view).offset(-TabbarSafeBottomMargin);
    }];
    [_jobTableView setBackgroundColor:[UIColor clearColor]];
    _jobTableView.bounces = YES;
    _jobTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [_jobTableView registerClass:[MineJobTableViewCell class] forCellReuseIdentifier:@"MineJobTableViewCell"];
    ...
    _jobTableView.showsVerticalScrollIndicator = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MineJobTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MineJobTableViewCell"];

    //更新cell，注意这里是根据section下标取值的
    [cell updateCellWithModel:self.jobArray[indexPath.section]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MineJobModel *slctModel = self.jobArray[indexPath.section];

    NSDictionary *sendDic = @{
        @"slctModel":slctModel
    };

    MineJobDetailViewController *detailVC = [MineJobDetailViewController new];
...
-(void)rightButtonClick
{
    if(isShowChinese){
        ...
        //更新数据源
        for(MineJobModel *model in self.jobArray){
...
- (void)queryMineWorkList
{
    ...
    [AvalonsoftHasNetwork avalonsoft_hasNetwork:^(bool has) {
        if (has) {
            ...
            [[AvalonsoftHttpClient avalonsoftHttpClient] requestWithAction:COMMON_SERVER_URL actionName:MINE_MY_WORK_LIST method:HttpRequestPost paramenters:root prepareExecute:^{
                ...
                @try {
                    if(responseModel.rescode == 200){
                       ...
                       [strongSelf.jobArray removeAllObjects];
                       for(int i=0; i<rspDataArray.count; i++){
                           MineJobModel *model = [MineJobModel modelWithDict:rspDataArray[i]];
                           [strongSelf.jobArray addObject:model];
                       }
