...
#import "SecurityModifyPwdCheckPwdVC.h"

@interface SecurityModifyPwdCheckPwdVC ()

{
    @private UIButton *backButton;              //左上角返回按钮
    @private UIView *navigationView;
    
    @private UIView *passwordOriginalView;      //原始密码视图
...
@implementation SecurityModifyPwdCheckPwdVC
...
-(void)initView
{
    ...
    NSMutableAttributedString *originalLabelString = [[NSMutableAttributedString alloc] initWithString:@"原始密码"attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size: 13],NSForegroundColorAttributeName: [UIColor colorWithRed:203/255.0 green:207/255.0 blue:217/255.0 alpha:1.0]}];
    originalLabel.attributedText = originalLabelString;
    
    
    //2.原始密码view
    passwordOriginalView = [UIView new];
    [self.view addSubview:passwordOriginalView];
    [passwordOriginalView mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(originalLabel.mas_bottom).offset(10);
        make.left.equalTo(self.view).offset((SCREEN_WIDTH-319)/2);
        make.size.mas_equalTo(CGSizeMake(319, 46));
    }];
    passwordOriginalView.layer.backgroundColor = [UIColor colorWithRed:243/255.0 green:246/255.0 blue:247/255.0 alpha:1.0].CGColor;
    passwordOriginalView.layer.cornerRadius = 23;
    
    //2-1.原始密码图标
    UIImageView *psdOriginalImageView = [UIImageView new];
    [passwordOriginalView addSubview:psdOriginalImageView];
    [psdOriginalImageView mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(passwordOriginalView).offset(14);
        make.left.equalTo(passwordOriginalView).offset(15);
        make.size.mas_equalTo(CGSizeMake(18, 18));
    }];
    [psdOriginalImageView setImage:[UIImage imageNamed:@"login_psd"]];
    
    //2-2.隐藏/显示原始密码按钮
    showPsdOriginalButton = [UIButton new];
    [passwordOriginalView addSubview:showPsdOriginalButton];
    [showPsdOriginalButton mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(passwordOriginalView).offset(15);
        make.right.equalTo(passwordOriginalView.mas_right).offset(-15);
        make.size.mas_equalTo(CGSizeMake(16, 16));
    }];
    //设置点击不变色
    showPsdOriginalButton.adjustsImageWhenHighlighted = NO;
    [showPsdOriginalButton setBackgroundImage:[UIImage imageNamed:@"login_psd_hide"] forState:UIControlStateNormal];
    
    //2-3.原始密码输入框
    psdOriginalTextField = [UITextField new];
    [passwordOriginalView addSubview:psdOriginalTextField];
    [psdOriginalTextField mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(passwordOriginalView).offset(8);
        make.left.equalTo(psdOriginalImageView.mas_right).offset(15);
        make.right.equalTo(showPsdOriginalButton.mas_left).offset(-10);
        make.height.equalTo(@30);
    }];
    psdOriginalTextField.secureTextEntry=YES;
    [psdOriginalTextField setTextAlignment:NSTextAlignmentLeft];
    psdOriginalTextField.clearButtonMode = UITextFieldViewModeNever;
    psdOriginalTextField.borderStyle = UITextBorderStyleNone;
    //设置占位文字样式
    NSMutableAttributedString *psdOriginalPlaceholderString = [[NSMutableAttributedString alloc] initWithString:@"输入原密码" attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size: 16],NSForegroundColorAttributeName: [UIColor colorWithRed:203/255.0 green:207/255.0 blue:217/255.0 alpha:1.0]}];
    psdOriginalTextField.attributedPlaceholder = psdOriginalPlaceholderString;
    //设置输入后文字样式
    [psdOriginalTextField setTextColor:RGBA_GGCOLOR(51, 51, 51, 1)];
    [psdOriginalTextField setFont:[UIFont fontWithName:@"PingFangSC-Regular" size:16]];
    ...
    [newPwdLabel mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(passwordOriginalView.mas_bottom).offset(15);
        make.left.equalTo(self.view).offset(28);
        make.size.mas_equalTo(CGSizeMake(120, 18));
    }];
    newPwdLabel.numberOfLines = 0;
    newPwdLabel.textAlignment = NSTextAlignmentLeft;
    newPwdLabel.alpha = 1.0;
    NSMutableAttributedString *newPwdLabelString = [[NSMutableAttributedString alloc] initWithString:@"新密码"attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size: 13],NSForegroundColorAttributeName: [UIColor colorWithRed:203/255.0 green:207/255.0 blue:217/255.0 alpha:1.0]}];
    newPwdLabel.attributedText = newPwdLabelString;
    ...
    [passwordView mas_makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(newPwdLabel.mas_bottom).offset(10);
        make.left.equalTo(self.view).offset((SCREEN_WIDTH-319)/2);
        make.size.mas_equalTo(CGSizeMake(319, 46));
    }];
...
{
    //显示原始密码按钮
    [showPsdOriginalButton addTarget:self action:@selector(showPsdOriginalButtonClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    //原始密码文本框监听
    [psdOriginalTextField addTarget:self action:@selector(textFieldDidChange) forControlEvents:UIControlEventEditingChanged];
...
#pragma mark - 显示原始密码按钮点击事件 -
-(void)showPsdOriginalButtonClick
{
    if(isShowOriginalPsd){
        //此时为明文显示，点击按钮切换为秘文显示
        isShowOriginalPsd = NO;
        [showPsdOriginalButton setBackgroundImage:[UIImage imageNamed:@"login_psd_hide"] forState:UIControlStateNormal];
        
        NSString *tempPwdStr = psdOriginalTextField.text;
        psdOriginalTextField.text = @"";    //这句代码可以防止切换的时候光标偏移
        psdOriginalTextField.secureTextEntry = YES;
        psdOriginalTextField.text = tempPwdStr;
        
    }else{
        //此时为秘文显示，点击按钮切换为明文显示
        isShowOriginalPsd = YES;
        [showPsdOriginalButton setBackgroundImage:[UIImage imageNamed:@"login_psd_show"] forState:UIControlStateNormal];
        
        NSString *tempPwdStr = psdOriginalTextField.text;
        psdOriginalTextField.text = @"";    //这句代码可以防止切换的时候光标偏移
        psdOriginalTextField.secureTextEntry = NO;
        psdOriginalTextField.text = tempPwdStr;
    }
}
...
{
    //判断注册按钮是否可点击
    if( ![psdOriginalTextField.text isEqualToString:@""] && ![psdTextField.text isEqualToString:@""] && ![psdConfirmTextField.text isEqualToString:@""]){
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [psdOriginalTextField resignFirstResponder];
