//
//  MyLikeStudyAbroadCompanyTableViewCell.m
//  Cafe
//
...
@implementation MyLikeStudyAbroadCompanyTableViewCell
...
- (void)updateCellWithModel:(MyLikeStudyAbroadCompanyModel *)model
{
    ...
    NSString *companyName = model.displayName;
    
    if (model.logo == NULL || [model.logo isEqualToString:@""]) {
        UIImage *icon = [UIImage imageNamed:@"home_foreign_school_icon"];
        _companyIconImageView.image = icon;
    } else {
        [_companyIconImageView sd_setImageWithURL:[NSURL URLWithString:[_F createFileLoadUrl:model.logo]]];
    }
