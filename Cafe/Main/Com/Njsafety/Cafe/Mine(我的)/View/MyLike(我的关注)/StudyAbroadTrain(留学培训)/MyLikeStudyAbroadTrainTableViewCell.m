//
//  MyLikeStudyAbroadTrainTableViewCell.m
//  Cafe
//
...
@implementation MyLikeStudyAbroadTrainTableViewCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if(self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]){
        ...
        //头像
        _trainIconImageView = [UIImageView new];
        [_backView addSubview:_trainIconImageView];
        [_trainIconImageView mas_makeConstraints:^(MASConstraintMaker *make){
        ...
        //名称
        _trainNameLabel = [UILabel new];
        [_backView addSubview:_trainNameLabel];
        [_trainNameLabel mas_makeConstraints:^(MASConstraintMaker *make){
            make.top.equalTo(_backView).offset((70 - 42)/2);
            make.left.equalTo(_trainIconImageView.mas_right).offset(10);
            make.right.equalTo(_backView).offset(-10-65-10);
            make.height.mas_equalTo(@42);
        }];
        _trainNameLabel.numberOfLines = 0;
        _trainNameLabel.textAlignment = NSTextAlignmentLeft;
        _trainNameLabel.alpha = 1.0;
...
- (void)updateCellWithModel:(MyLikeStudyAbroadTrainModel *)model
{
    ...
    NSString *trainName = model.instnameen;
    
    if (model.logo == NULL || [model.logo isEqualToString:@""]) {
        UIImage *icon = [UIImage imageNamed:@"home_foreign_school_icon"];
        _trainIconImageView.image = icon;
    } else {
        [_trainIconImageView sd_setImageWithURL:[NSURL URLWithString:[_F createFileLoadUrl:model.logo]]];
    }

    
    //标题
    NSMutableAttributedString *nameString = [[NSMutableAttributedString alloc] initWithString:trainName attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Medium" size: 15],NSForegroundColorAttributeName: [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0]}];
    _trainNameLabel.attributedText = nameString;
